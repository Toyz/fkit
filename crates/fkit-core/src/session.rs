    ...
    loop {
        ...
        match msg {
            ...
            Msg::PushRef { branch, tip, force } => {
                ...
                //
                // Gaps left by an interrupted transfer are filled inside that
                // call, before it reports itself finished. Doing it out here
                // instead sent a fresh request after the exchange had already
                // been closed, at a peer that had stopped listening for one.
                let stats = fetch_closure(host.store(), t, &[tip])?;
